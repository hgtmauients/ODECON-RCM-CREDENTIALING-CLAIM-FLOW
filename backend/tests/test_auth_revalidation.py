...
async def test_get_current_user_rejects_revoked_token(monkeypatch):
    ...
    request = SimpleNamespace(headers={}, client=SimpleNamespace(host="127.0.0.1"))
    creds = HTTPAuthorizationCredentials(scheme="Bearer", credentials=token)
    db = _FakeDB([SimpleNamespace(id=uuid.uuid4()), SimpleNamespace(id=uuid.uuid4(), is_active=True, roles=["admin"])])
    monkeypatch.setattr(auth_module, "is_token_revoked", AsyncMock(return_value=True))
